...
class P1DGPoissonSolver:
    ...
    """
    def __init__(self, mesh, penalty_param=10.0):
        self.mesh = mesh
    ...
    def assemble_system(self, f_func, g_func):
        """
        ...
        for edge_id in range(len(self.mesh.edges)):
            ...
            if len(cells) == 2:  # Interior edge
                cell_i, cell_j = cells
                n = self.mesh.edge_normal(edge_id, cell_i)  # normal pointing from i to j
                ...
                g_val = g_func(edge_mid[0], edge_mid[1])
    ...
    def solve(self, f_func, g_func):
        ...
        A, b = self.assemble_system(f_func, g_func)
        u_dofs = spsolve(A, b)
        return u_dofs
